...
using WebBanHang.Models.Payment;
    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null && cart.items.Any())
                {
                    ...
                    //sendMail cho customer
                    string contentCustomer = System.IO.File.ReadAllText(Server.MapPath("~/Content/template/send2.html")); // lay duong dan
                    contentCustomer = contentCustomer.Replace("{{MaDonHang}}", order.Code);
                    contentCustomer = contentCustomer.Replace("{{SanPham}}", strSanPham);
                    contentCustomer = contentCustomer.Replace("{{DonGia}}", thanhtien.ToString());
                    contentCustomer = contentCustomer.Replace("{{TongTien}}", tongtien.ToString());
                    contentCustomer = contentCustomer.Replace("{{NgayDat}}", DateTime.Now.ToString("dd/MM/yyyy"));
                    contentCustomer = contentCustomer.Replace("{{TenKhanhHang}}", order.CustomerName);
                   ...
                   // WebBanHang.Models.Common.Commom.SendMail("DuyPhanHome", "Đơn Hàng #" + order.Code, contentCustomer, req.Email);
                    ...
                    contentAdmin = contentAdmin.Replace("{{DonGia}}", thanhtien.ToString());
                    contentAdmin = contentAdmin.Replace("{{TongTien}}", tongtien.ToString());
                    contentAdmin = contentAdmin.Replace("{{NgayDat}}", DateTime.Now.ToString("dd/MM/yyyy"));
                    contentAdmin = contentAdmin.Replace("{{TenKhanhHang}}", order.CustomerName);
                    contentAdmin = contentAdmin.Replace("{{DiaChi}}", order.Address);
                    contentAdmin = contentAdmin.Replace("{{Phone}}", order.Phone);
                    contentAdmin = contentAdmin.Replace("{{Email}}", order.Email);
                    //WebBanHang.Models.Common.Commom.SendMail("DuyPhanHome", "Đơn Hàng #" + order.Code, contentAdmin, ConfigurationManager.AppSettings["EmailAdmin"]);
                    if (cart == null)
                    {
                        return Json(new { Success = false, Code = -10, Url = "", Error = "Cart is null" });
                    }

                    if (cart.items == null)
                    {
                        return Json(new { Success = false, Code = -11, Url = "", Error = "Cart.items is null" });
                    }
                    try
                    {
                        cart.ClearCart();
                    ...
                    {
                        return Json(new { Success = false, Code = -12, Url = "", Error = "ClearCart error: " + ex.Message });
                    }
        ...
        public ActionResult AddToCart(int id,int quantity)
        {
            ...
            if(checkProduct!=null)
            {
                ...
                if (checkProduct.PriceSale != null)
                {
        ...
        private void GiamSoLuongTrongKho(Order order)
        {
        ...
        public ActionResult ExportPdf(int id)
        {
            ...
            {
                FileName = $"PhieuMuaHang_{order.Code}.pdf",
